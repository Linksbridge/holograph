...
const fetchLiveData = async tableName => {
  ...
  const url = dataQueryUrl.includes('{table}') ? dataQueryUrl.replace('{table}', encodeURIComponent(tableName)) : `${dataQueryUrl.replace(/\/$/, '')}/${encodeURIComponent(tableName)}`;
...
const ZoneContent = ({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ref: containerRef,
    style: containerBaseStyle
  }, Adapter && /*#__PURE__*/React.createElement(React.Suspense, {
...
const DashboardViewer = ({
  ...
  const containerRef = React.useRef(null); // outer container — measured for width/height
...
const D3Adapter = ({
  ...
  const renderBarChart = (svg, chartData) => {
    ...
    if (isMulti) {
      const xScale = d3__namespace.scaleBand().domain(chartData.map(d => d.label)).range([0, innerWidth]).padding(0.2);
      const xInner = d3__namespace.scaleBand().domain(cols).range([0, xScale.bandwidth()]).padding(0.05);
      const allVals = cols.flatMap(c => chartData.map(d => d[c] ?? 0));
      const yScale = d3__namespace.scaleLinear().domain([0, (d3__namespace.max(allVals) || 1) * 1.1]).range([innerHeight, 0]);
      const xAxisG = g.append('g').attr('transform', `translate(0,${innerHeight})`).call(d3__namespace.axisBottom(xScale));
      ...
      const yAxisG = g.append('g').call(d3__namespace.axisLeft(yScale).ticks(4));
      yAxisG.selectAll('text').attr('fill', colors.text).style('font-size', `${xFontSize}px`);
      yAxisG.select('.domain').attr('stroke', colors.grid);
      g.append('g').attr('class', 'grid').call(d3__namespace.axisLeft(yScale).ticks(4).tickSize(-innerWidth).tickFormat('')).selectAll('line').attr('stroke', colors.grid).attr('stroke-opacity', 0.4).attr('stroke-dasharray', '2,2');
    ...
    const col0 = cols?.[0];
    const xScale = d3__namespace.scaleBand().domain(chartData.map(d => d.label)).range([0, innerWidth]).padding(0.3);
    const yScale = d3__namespace.scaleLinear().domain([0, (d3__namespace.max(chartData, d => d[col0] ?? d.value ?? 0) || 1) * 1.1]).range([innerHeight, 0]);
    const xAxisG = g.append('g').attr('class', 'x-axis').attr('transform', `translate(0,${innerHeight})`).call(d3__namespace.axisBottom(xScale));
    xAxisG.selectAll('text').attr('fill', colors.text).style('font-size', `${xFontSize}px`).style('text-anchor', width < 150 ? 'end' : 'middle').attr('transform', width < 150 ? 'rotate(-45)' : '');
    xAxisG.select('.domain').attr('stroke', colors.grid);
    const yAxisG = g.append('g').attr('class', 'y-axis').call(d3__namespace.axisLeft(yScale).ticks(4));
    yAxisG.selectAll('text').attr('fill', colors.text).style('font-size', `${xFontSize}px`);
    yAxisG.select('.domain').attr('stroke', colors.grid);
    g.append('g').attr('class', 'grid').call(d3__namespace.axisLeft(yScale).ticks(4).tickSize(-innerWidth).tickFormat('')).selectAll('line').attr('stroke', colors.grid).attr('stroke-opacity', 0.4).attr('stroke-dasharray', '2,2');
    g.select('.grid .domain').remove();
  ...
  const renderLineChart = (svg, chartData) => {
    ...
    const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);
    const xFontSize = Math.max(8, Math.min(10, width / 40));
    const xScale = d3__namespace.scalePoint().domain(chartData.map(d => d.label)).range([0, innerWidth]);
    ...
    activeCols.forEach((col, ci) => {
      ...
      g.append('path').datum(chartData).attr('fill', `${color}20`).attr('d', d3__namespace.area().x(d => xScale(d.label)).y0(innerHeight).y1(d => yScale(getValue(d))).curve(d3__namespace.curveMonotoneX));
      const path = g.append('path').datum(chartData).attr('fill', 'none').attr('stroke', color).attr('stroke-width', 2.5).attr('d', d3__namespace.line().x(d => xScale(d.label)).y(d => yScale(getValue(d))).curve(d3__namespace.curveMonotoneX));
  ...
  const renderAreaChart = (svg, chartData) => {
    ...
    activeCols.forEach((col, ci) => {
      ...
      const areaPath = g.append('path').datum(chartData).attr('fill', `url(#${gradId})`).attr('d', d3__namespace.area().x(d => xScale(d.label)).y0(innerHeight).y1(d => yScale(getValue(d))).curve(d3__namespace.curveMonotoneX));
      areaPath.attr('opacity', 0).transition().duration(800).attr('opacity', 1);
      g.append('path').datum(chartData).attr('fill', 'none').attr('stroke', color).attr('stroke-width', 2).attr('d', d3__namespace.line().x(d => xScale(d.label)).y(d => yScale(getValue(d))).curve(d3__namespace.curveMonotoneX));
    });
  ...
  const renderPieChart = (svg, chartData, isDonut) => {
    ...
    const pie = d3__namespace.pie().value(d => getValue(d)).sort(null);
    const arc = d3__namespace.arc().innerRadius(innerRadius).outerRadius(radius);
    const arcHover = d3__namespace.arc().innerRadius(innerRadius).outerRadius(radius + 8);
    ...
    slices.append('path').attr('d', arc).attr('fill', (d, i) => colorPalette[i % colorPalette.length]).attr('stroke', colors.background).attr('stroke-width', 2).style('opacity', 0).transition().duration(500).delay((d, i) => i * 50).style('opacity', 1).attrTween('d', function (d) {
      const interpolate = d3__namespace.interpolate({
        startAngle: 0,
        endAngle: 0
      }, d);
    ...
    slices.select('path').on('mouseenter', function (event, d) {
      d3__namespace.select(this).transition().duration(100).attr('d', arcHover);
    }).on('mouseleave', function (event, d) {
      d3__namespace.select(this).transition().duration(100).attr('d', arc);
    });
    ...
    if (isDonut) {
      const total = d3__namespace.sum(chartData, d => getValue(d));
  ...
  const renderScatterChart = (svg, chartData) => {
    ...
    if (innerWidth <= 0 || innerHeight <= 0) return;
    const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);
    const xScale = d3__namespace.scaleLinear().domain([0, chartData.length + 1]).range([0, innerWidth]);
    const yScale = d3__namespace.scaleLinear().domain([0, (d3__namespace.max(chartData, d => getValue(d)) || 1) * 1.1]).range([innerHeight, 0]);
    const xFontSize = Math.max(8, Math.min(10, width / 40));

    // Axes
    const xAxisG = g.append('g').attr('transform', `translate(0,${innerHeight})`).call(d3__namespace.axisBottom(xScale).ticks(chartData.length));
    xAxisG.selectAll('text').attr('fill', colors.text).style('font-size', `${xFontSize}px`);
    xAxisG.select('.domain').attr('stroke', colors.grid);
    const yAxisG = g.append('g').call(d3__namespace.axisLeft(yScale).ticks(4));
    yAxisG.selectAll('text').attr('fill', colors.text).style('font-size', `${xFontSize}px`);
...
chart_js.Chart.register(chart_js.CategoryScale, chart_js.LinearScale, chart_js.PointElement, chart_js.LineElement, chart_js.BarElement, chart_js.ArcElement, chart_js.RadialLinearScale, chart_js.Title, chart_js.Tooltip, chart_js.Legend, chart_js.Filler, chartjsChartGeo.BubbleMapController, chartjsChartGeo.ProjectionScale, chartjsChartGeo.GeoFeature, chartjsChartGeo.SizeScale);
...
const BubbleMapChart = ({
  ...
  }, /*#__PURE__*/React.createElement(reactChartjs2.Chart, {
    ref: chartRef,
    type: "bubbleMap",
    data: chartData,
    options: options
...
const CHART_COMPONENTS = {
  [CHART_TYPES.CHARTJS_LINE]: reactChartjs2.Line,
  [CHART_TYPES.CHARTJS_BAR]: reactChartjs2.Bar,
  [CHART_TYPES.CHARTJS_PIE]: reactChartjs2.Pie,
  [CHART_TYPES.CHARTJS_DOUGHNUT]: reactChartjs2.Doughnut,
  [CHART_TYPES.CHARTJS_RADAR]: reactChartjs2.Radar,
  [CHART_TYPES.CHARTJS_POLAR]: reactChartjs2.PolarArea
};
const ChartJsAdapter = ({
  ...
  const ChartComponent = CHART_COMPONENTS[chartType] || reactChartjs2.Line;
...
({
  [CHART_TYPES.NIVO_LINE]: line.ResponsiveLine,
  [CHART_TYPES.NIVO_BAR]: bar.ResponsiveBar,
  [CHART_TYPES.NIVO_PIE]: pie.ResponsivePie,
  [CHART_TYPES.NIVO_CHOROPLETH]: geo.ResponsiveChoropleth
});
...
const getNivoChartComponent = nivoChartType => {
  switch (nivoChartType) {
    case NIVO_CHART_TYPES.LINE:
      return line.ResponsiveLine;
    case NIVO_CHART_TYPES.BAR:
      return bar.ResponsiveBar;
    case NIVO_CHART_TYPES.PIE:
      return pie.ResponsivePie;
    case NIVO_CHART_TYPES.CHOROPLETH:
      return geo.ResponsiveChoropleth;
    default:
      return line.ResponsiveLine;
  }
};
...
const NivoAdapter = ({
  ...
  const [choroplethZoom, setChoroplethZoom] = React.useState(1);
  ...
  const choroplethConfig = React.useMemo(() => {
...
exports.CHART_COLUMN_SCHEMA = CHART_COLUMN_SCHEMA;
exports.CHART_LIBRARIES = CHART_LIBRARIES;
exports.CHART_TYPES = CHART_TYPES;
exports.CHART_TYPE_LIBRARY = CHART_TYPE_LIBRARY;
exports.COLOR_THEMES = COLOR_THEMES;
exports.COMPONENT_TYPES = COMPONENT_TYPES;
exports.DEFAULT_CHART_TYPE = DEFAULT_CHART_TYPE;
exports.DashboardViewer = DashboardViewer;
exports.LEGEND_POSITIONS = LEGEND_POSITIONS;
exports.SIZING_MODES = SIZING_MODES;
exports.THEMES = THEMES;
exports.createInitialDashboard = createInitialDashboard;
exports.createZoneConfig = createZoneConfig;
exports.default = DashboardViewer;
