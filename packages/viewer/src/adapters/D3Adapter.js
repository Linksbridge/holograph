...
const D3Adapter = ({ data, valueColumns = [], theme = 'default', width = 400, height = 300, title, chartType = CHART_TYPES.D3_BAR, legend, resolvedStyles = {} }) => {
  ...
  const renderBarChart = (svg, chartData) => {
    ...
    if (isMulti) {
      ...
      cols.forEach((col, ci) => {
        g.selectAll(`.bar-${ci}`).data(chartData).enter().append('rect')
          ...
          .attr('y', innerHeight).attr('width', xInner.bandwidth()).attr('height', 0)
          .attr('fill', colorPalette[ci % colorPalette.length]).attr('rx', 2)
    ...
    g.selectAll('.bar').data(chartData).enter().append('rect')
      .attr('class', 'bar').attr('x', (d) => xScale(d.label)).attr('y', innerHeight)
      .attr('width', xScale.bandwidth()).attr('height', 0).attr('fill', colors.primary).attr('rx', 2)
      .transition().duration(500).delay((d, i) => i * 20)
  ...
  const renderLineChart = (svg, chartData) => {
    ...
    activeCols.forEach((col, ci) => {
      ...
      g.append('path').datum(chartData).attr('fill', `${color}20`)
      ...
      g.selectAll(`.point-${ci}`).data(chartData).enter().append('circle')
        .attr('class', `point-${ci}`).attr('cx', (d) => xScale(d.label)).attr('cy', (d) => yScale(getValue(d)))
  ...
  const addTitleAndLegend = (svg, chartTitle, defaultTitle, forceLegend = false, seriesNames = null) => {
    ...
    if (title && width > 100) {
      svg.append('text')
        .attr('x', width / 2).attr('y', 16).attr('text-anchor', 'middle').attr('fill', colors.text)
        .style('font-size', `${Math.max(9, Math.min(12, width / 30))}px`)
        .style('font-weight', 'bold').style('font-family', fontFamily).text(title);
    }
